    ...
    async def list_models(self) -> tuple[list[ModelInfo], list[ModelInfo]]:
        ...
        for row in data:
            ...
            info = ModelInfo(
                id=model_id,
                name=model_id,
                type="embed" if _is_embed_id(model_id) else "chat",
            )
            (embed if info.type == "embed" else chat).append(info)
        chat.sort(key=lambda m: m.id)
        embed.sort(key=lambda m: m.id)
        return chat, embed


def _is_embed_id(model_id: str) -> bool:
    return model_id.startswith("text-embedding-") or "embed" in model_id.lower()
